...
const FallbackContract: FallbackContractContract =
  artifacts.require("FallbackContract");
const EthereumPaymentCollector: EthereumPaymentCollectorContract =
  artifacts.require("EthereumPaymentCollector");
...
async function createSepoliaTransactions() {
  ...
  const tx1 = await createSimpleTransaction(
    randomEthereumAddress,
    10,
    "0x0123456789"
  );
  await tx1.wait();
  const tx2 = await createSimpleTransaction(
    fallback.address,
    10,
    "0x9876543210"
  );
  ...
  console.log(
    JSON.stringify(
      await prepareAttestationResponse("EVMTransaction", "eth", "testETH", {
...
async function executeStateConnectorProof(txs: string[]) {
  ...
  const responses = await Promise.all(
    txs.map(async (tx) => {
      const req = await prepareAttestationRequest(
        "EVMTransaction",
        "eth",
        "testETH",
        {
  ...
  while (
    Number(await stateConnector.lastFinalizedRoundId()) <
